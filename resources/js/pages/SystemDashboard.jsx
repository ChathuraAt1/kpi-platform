...
export default function SystemDashboard() {
    ...
    return (
        <div className="space-y-10 pb-20">
            <header className="relative p-10 rounded-4xl bg-slate-900 text-white overflow-hidden shadow-2xl border border-white/5 group">
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            <span className="px-3 py-1 bg-indigo-500/20 backdrop-blur-md rounded-full text-[10px] font-black text-indigo-400 uppercase tracking-widest border border-indigo-400/20">Operational Intel</span>
                        </div>
                        <h2 className="text-4xl font-black tracking-tight">System <span className="text-indigo-400">Governance</span></h2>
                        <p className="text-slate-400 font-medium max-w-sm mt-1">Strategic oversight of organizational performance cycles and technical infrastructure.</p>
                    </div>
                </div>
                <div className="absolute top-[-50%] right-[-10%] w-[60%] h-[200%] bg-indigo-600/10 blur-[120px] rounded-full rotate-45 transform transition-transform duration-1000 group-hover:rotate-90"></div>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    ...
                    <div key={i} className="bg-white p-8 rounded-4xl border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
                        ...
                        <div className="text-[10px] font-bold text-slate-400 mt-4 uppercase tracking-tighter italic">{s.sub}</div>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="flex-1 flex items-end gap-4 px-2">
                        {[40, 65, 45, 80, 55, 90, 70, 85, 60, 95].map((h, i) => (
                            <div key={i} className="flex-1 bg-slate-50 rounded-t-2xl relative group/bar hover:bg-indigo-50 transition-colors">
                                <div className="absolute bottom-0 w-full bg-linear-to-t from-indigo-600 to-indigo-400 rounded-2xl shadow-lg shadow-indigo-500/20 group-hover/bar:from-indigo-500 group-hover/bar:to-violet-500 transition-all duration-500" style={{ height: `${h}%` }}></div>
                                <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[8px] font-black px-2 py-1 rounded opacity-0 group-hover/bar:opacity-100 transition-opacity whitespace-nowrap z-10">{h}% Velocity</div>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                {/* API Pulse Mockup */}
                <div className="bg-slate-900 rounded-[3rem] shadow-2xl p-10 h-96 flex flex-col text-white relative overflow-hidden">
                    <div className="relative z-10 flex justify-between items-center mb-10">
                        <h3 className="text-xl font-black tracking-tight">API Node <span className="text-indigo-400">Heartbeat</span></h3>
                    ...
                    <div className="relative z-10 flex-1 flex flex-col justify-center">
                        <svg className="w-full h-32 text-indigo-500/30" viewBox="0 0 400 100" preserveAspectRatio="none">
