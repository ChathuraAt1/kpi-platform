        ...
        <div className="space-y-10 pb-20">
                ...
                <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8 text-white">
                    <div className="space-y-2">
                        <div className="flex items-center gap-3">
                            <span className="px-3 py-1 bg-orange-500/20 backdrop-blur-md rounded-full text-[10px] font-black text-orange-300 uppercase tracking-[0.2em] border border-orange-400/20">Executive Suite</span>
                            <span className="w-1 h-1 bg-white/30 rounded-full"></span>
                            <span className="text-neutral-400 text-xs font-medium uppercase tracking-wider">Performance Governance</span>
                        </div>
            ...
            <div className="flex items-center gap-2 p-1.5 bg-neutral-200/50 dark:bg-gray-700/50 backdrop-blur-md rounded-md w-fit self-start border border-neutral-200 dark:border-gray-600">
                {[
                    ...
                    >
                        <svg className={`w-4 h-4 ${tab === t.id ? 'text-orange-500' : 'text-neutral-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                {tab === 'governance' && (
                    <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                        <div className="px-10 py-8 border-b border-slate-50 flex justify-between items-center bg-neutral-50/50">
                            <h3 className="text-xl font-black text-black dark:text-white tracking-tight">Review & Publication Queue</h3>
                            <div className="flex gap-4">
                                <div className="px-4 py-1.5 rounded-lg bg-orange-50 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 text-[10px] font-black uppercase tracking-widest border border-orange-100 dark:border-orange-800 italic">OFFICIAL RECORD</div>
                            </div>
                        </div>

                        {loading ? (
                            <div className="p-20 text-center animate-pulse text-neutral-400 font-bold uppercase tracking-[0.2em] text-xs transition-opacity duration-500 font-mono">Loading official stream...</div>
                        ) : evaluations.length === 0 ? (
                            <div className="p-20 text-center text-slate-300 dark:text-gray-400 font-bold italic">No evaluations found on record.</div>
                        ) : (
                            <div className="overflow-x-auto">
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-50 dark:divide-gray-700">
                                        {evaluations.map((ev) => (
                                            <tr key={ev.id} className="hover:bg-slate-50/80 dark:hover:bg-gray-700/50 transition-colors group">
                                                <td className="px-10 py-6">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <span className="font-bold text-black dark:text-white">{ev.user?.name}</span>
                                                        <span className="text-[10px] text-neutral-400 dark:text-gray-400 font-bold uppercase tracking-wider">{ev.user?.email}</span>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-10 py-6">
                                                    <span className="font-black text-neutral-700 dark:text-gray-300">{ev.year} / {String(ev.month).padStart(2, '0')}</span>
                                                </td>
                                                <td className="px-10 py-6">
                                                    <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border ${ev.status === 'published' ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-800' :
                                                            ev.status === 'approved' ? 'bg-orange-50 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 border-orange-100 dark:border-orange-800' :
                                                ...
                                                <td className="px-10 py-6">
                                                    {ev.score !== null ? (
                                                        <span className="text-xl font-black text-orange-600 dark:text-orange-400 tracking-tighter">{ev.score}<span className="text-[10px] text-neutral-400 dark:text-gray-400 font-normal"> / 10</span></span>
                                                    ) : (
                                                        <span className="text-xs font-bold text-slate-300 dark:text-gray-500 italic">Pending Finalization</span>
                                                    )}
                                                </td>
                                                <td className="px-10 py-6 text-right">
                                                    {ev.status === "approved" ? (
                                                        ...
                                                        <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">PUBLISHED</span>
                                                    ) : (
                                                        <span className="text-[10px] font-black text-neutral-400 dark:text-gray-500 uppercase tracking-widest">Awaiting Superior</span>
                                                    )}
                                                </td>
                                            </tr>
                                        ))}
                ...
                {tab === 'framework' && (
                    <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                        <div className="px-10 py-8 border-b border-slate-50 flex justify-between items-center bg-neutral-50/50">
                            <h3 className="text-xl font-black text-black dark:text-white tracking-tight">KPI Framework Definition</h3>
                        ...
                        <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {categories.map(cat => (
                                <div key={cat.id} className="p-5 bg-slate-50 dark:bg-gray-800 rounded-lg border border-slate-100 dark:border-gray-700 relative group overflow-hidden">
                                    <div className="relative z-10">
                                        ...
                                        <h4 className="font-black text-black dark:text-white mb-2">{cat.name}</h4>
                                        <p className="text-sm text-neutral-500 dark:text-gray-400 leading-relaxed mb-6 font-medium">{cat.description || 'No description provided for this category.'}</p>
                                        <div className="flex gap-2">
                                            <button className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-400 dark:text-gray-400 hover:text-orange-600 dark:hover:text-orange-400 transition-colors">Edit Framework</button>
                                        </div>
                                    </div>
