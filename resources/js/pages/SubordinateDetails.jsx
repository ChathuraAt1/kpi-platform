...
export default function SubordinateDetails() {
    ...
    if (loading || !sub) return <div className="p-20 text-center animate-pulse text-indigo-500 font-black uppercase tracking-widest">Loading Subordinate Matrix...</div>;

    return (
        <div className="space-y-10 pb-20 max-w-6xl mx-auto">
            <header className="relative p-10 rounded-4xl bg-slate-900 text-white overflow-hidden shadow-2xl border border-indigo-500/10">
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div className="flex items-center gap-6">
                        <div className="w-20 h-20 rounded-3xl bg-linear-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-3xl font-black shadow-2xl shadow-indigo-500/30">
                            {sub.name.charAt(0)}
                        </div>
                        <div>
                            ...
                            <p className="text-slate-400 font-medium">{sub.role} • {sub.email}</p>
                        </div>
                    </div>
                    <div className="flex gap-4">
                        <div className="bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-white/5 text-center min-w-[140px]">
                            <div className="text-3xl font-black text-indigo-400">8.4</div>
                            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1">Avg Score</div>
                        </div>
                    </div>
                </div>
                <div className="absolute top-0 right-0 w-[50%] h-full bg-linear-to-l from-indigo-500/10 to-transparent pointer-events-none"></div>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            {recentLogs.map(log => (
                                <div key={log.id} className="flex gap-6 relative group">
                                    <div className="flex flex-col items-center">
                                        <div className="w-3 h-3 rounded-full bg-indigo-500 ring-4 ring-indigo-50 z-10"></div>
                                    ...
                                    <div className="flex-1 pb-10">
                                        ...
                                        <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 group-hover:border-indigo-200 group-hover:bg-white transition-all shadow-sm hover:shadow-lg">
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Weight: {log.weight || 10}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div className="space-y-8">
                            ...
                            {[1, 2, 3].map(i => (
                                <div key={i} className="space-y-2">
                                    <div className="flex justify-between items-end px-1">
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap overflow-hidden text-ellipsis">Technical Velocity</span>
                    ...
                    {/* Shift Intelligence */}
                    <div className="bg-indigo-600 rounded-[2.5rem] shadow-xl shadow-indigo-600/20 p-8 text-white">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-indigo-200">Shift Configuration</span>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <div className="text-[10px] font-black text-indigo-300 uppercase tracking-widest opacity-60">Arrival</div>
                                <div className="text-xl font-black">{sub.work_start_time || '09:00'}</div>
                            </div>
                            <div>
                                <div className="text-[10px] font-black text-indigo-300 uppercase tracking-widest opacity-60">Departure</div>
